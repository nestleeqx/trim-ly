...
import { useAliasCheck } from './useAliasCheck'
...
export const useLinkForm = ({ link, onChange }: UseLinkFormProps) => {
	...
	const hasAdvancedParams = useMemo(() => {
		return Boolean(link.expirationDate) || Boolean(link.hasPassword)
	}, [link.expirationDate, link.hasPassword])
	...
	const handleFieldChange = useCallback(
		<K extends keyof LinkEditFormData>(
			field: K,
